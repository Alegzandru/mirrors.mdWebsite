...
const CustomButtonGroupAsArrows3 = ({ next, previous }) => {
  ...
  return (
    <div className="absolute left-0 w-full px-container-sm md:px-container-md lg:px-container-lg xl:px-container-xl flex flex-row justify-between items-center mx-auto mt-5">
      <div 
        className="h-8 w-8 rounded-full hover:border-transparent border-ui-blueishGrey border-2 text-ui-blueishGrey flex flex-row justify-center items-center cursor-pointer group transition duration-300" onClick={previous}
        onMouseOver={() => setLeftHovered(1)}
        onMouseLeave={() => setLeftHovered(0)}
      >
        {!leftHovered ? 
        ...
        {leftHovered ?
          <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-instatext" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" />
          </svg>
          :
          ""
      ...
      <div 
        className="h-8 w-8 rounded-full hover:border-transparent border-ui-blueishGrey border-2 text-ui-blueishGrey flex flex-row justify-center items-center cursor-pointer transition duration-300" onClick={next}
        onMouseOver={() => setRightHovered(1)}
        onMouseLeave={() => setRightHovered(0)}
      >
        {!rightHovered ? 

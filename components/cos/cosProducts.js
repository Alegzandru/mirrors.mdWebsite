...
export default function CosProducts({lang}){
  ...
  const onSubmit = async (data) => {
    if(data.height !== undefined && data.width !== undefined){
      ...
      const size = await getSize(data.height, data.width)

      changeProduct.size = size
      changeProduct.price = Math.round( getPrice(changeProduct.product, data) * ( 1 + coeficientFinder(data, changeProduct.product, roDomain)))
      ...
      changeProduct.price = Math.round( getPrice(changeProduct.product, changeProduct.size) * ( 1 + coeficientFinder(changeProduct.size, changeProduct.product, roDomain)))
                    ...
                    <div className="py-6 px-2 lg:px-8 md:w-cart-md lg:w-cart-lg border-l-0 border-b md:border-b-0 border-t-0 border-r-2 border-ui-darkGrey text-lg-14 text-type-grey">
                      ...
                      <div 
                        ...
                        onClick={async () => {
                          ...
                          const categoryResRaw = await fetch(`${API_URL}/categories?name_eq=${category}`)
                          const categoryRes = await categoryResRaw.json()
                          const optionsRaw = categoryRes[0].add_ons
                          setPopupProduct(product)
                          const addOns = product.product.add_ons.length ? product.product.add_ons : optionsRaw
                          await setOptionVariants(addOns)
                          const optionNamesUnfiltered = addOns.map((option) => {
                            if(option.group){
                              return option.group
                            }
                            else {
                          ...
                          const optionNamesUnfilteredRu = addOns.map((option) => {
                            if(option.group){
                          ...
                          const optionNamesUnfilteredEn = addOns.map((option) => {
                            if(option.group){
